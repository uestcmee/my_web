...
def pred_line(day: str):
    ...
    today_line = bond_china_yield_df[
        bond_china_yield_df["曲线名称"] == "中债中短期票据收益率曲线(AAA)"
        ].iloc[0]
    ...
    for one in day_line.index:
        if one in today_line.index:
            ...
            day_line.loc[one] = round(
                (rate0 + (one - year0) / (year1 - year0) * (rate1 - rate0)), 4
            )
    return day_line
...
def plot_prepare(small_df, day: str):
    ...
    def pred_rate(year):  # 计算预测收益率
        if year <= 1 and year < 0.5:
            ...
            y0 = int(year)
            y1 = y0 + 1
        rate = day_line.loc[y0] + (year - y0) * (
                day_line.loc[y1] - day_line.loc[y0]
        ) / (y1 - y0)
        return rate

    small_df = small_df[(small_df["year"] > 0.5) & (small_df["year"] < 5)]
    small_df["real_ytm"] = small_df["收益率"].apply(
        lambda x: float(re.findall("\d+\.\d+|(?<!-)\d+", x)[0])
    )
...
def deal_process_func(file_name="2020年06月25日周四.txt"):
    ...
    for i, line in enumerate(text.split("\n")):
        ...
        if len(line) >= 5:
            ...
            try:
                ...
                ytm = line[
                    [
                        re.search("\d+\.\d+|(?<!-)\d+", i) != None for i in line[3:]
                    ].index(True)
                    + 3
                    ].replace(
                    "%", ""
                ...
                name = line[
                    [re.search("[^\x00-\xff]+", i) != None for i in line[1:]].index(
                        True
                    )
                    + 1
                    ]  # 找名称
    ...
    fenlei_df = {
        i: pd.DataFrame(fenlei[i], columns="剩余期限 简称 收益率 评级".split(" "))
        for i in fenlei.keys()
    }
    ...
    for key in fenlei_df:  # 每一个债券类型
        ...
        fenlei_df[key] = pd.DataFrame(df).drop_duplicates(
            keep="first"
        )  # .drop('day', axis=1)  # 去掉辅助列
...
def output_excel(file_name: str, fenlei_df):
    ...
    def write_area(sht, coor, write_df):
        ...
        for i, row in enumerate(
                sht.iter_rows(
                    min_row=min_row, min_col=min_col, max_col=max_col, max_row=max_row
                )
        ):
